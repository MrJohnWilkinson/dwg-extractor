...
- **`app/core/logger.py`** - Centralized logging configuration. Primary output to stdout for real-time LLM agent monitoring (INFO level), secondary output to file for detailed debugging (DEBUG level).
...
5. **Create logging configuration in `app/core/logger.py`**
   ...
   ```python
   """
   Centralized logging configuration for DWG Block Extractor.
   Logs to stdout (for real-time monitoring by LLM agents) and file (for persistence).
   """
   import logging
   import sys
   ...
   LOG_FILE = LOGS_DIR / f'dwg_extractor_{datetime.now():%Y%m%d}.log'

   def setup_logger(name: str, level: int = logging.INFO) -> logging.Logger:
       """
       Configure and return a logger with stdout and file handlers.

       IMPORTANT: Logs are written to stdout for real-time monitoring by LLM agents.
       File logging is secondary for persistence and debugging.
       ...
       # PRIMARY: stdout handler - for real-time LLM agent monitoring
       # All logs go to stdout so agents can see them immediately
       ...
       # SECONDARY: File handler - for persistence (optional)
       # Detailed logs saved to file with DEBUG level
       ...
       # Add handlers - stdout first (primary)
   ...
   **Key logging levels used in the application:**
   - `DEBUG`: Detailed information for diagnosing issues (only in file)
   - `INFO`: General informational messages (stdout + file)
   - `WARNING`: Warning messages for unexpected situations (stdout + file)
   - `ERROR`: Error messages for failures (stdout + file)

### Success Criteria
- [ ] Directory structure matches specification including `logs/` directory
- [ ] All dependencies install without errors using `uv sync`
- [ ] Python version is 3.8+
- [ ] Logging configuration creates log files successfully
...
2. **Create `app/core/extractor.py`**
   ...
   ```python
   """DWG/DXF block extraction module."""
   import ezdxf
   from pathlib import Path
   ...
   def extract_blocks(file_path: str) -> dict[str, int]:
       ...
       try:
           # Load document
           logger.debug("Loading DWG/DXF document")
           doc = ezdxf.readfile(file_path)

           # Get modelspace
           msp = doc.modelspace()
           logger.debug(f"Accessing modelspace with {len(msp)} entities")
           ...
           for entity in msp:
               if entity.dxftype() == 'INSERT':
                   ...
                   logger.debug(f"Found INSERT: {block_name}")

           logger.info(f"Extraction complete: {len(block_counts)} unique blocks, "
                      f"{sum(block_counts.values())} total insertions")
           return block_counts

       except ezdxf.DXFStructureError as e:
           logger.error(f"Invalid DXF structure: {e}")
           raise ValueError(f"Invalid or corrupted DWG/DXF file: {e}")
...
3. **Create `app/core/excel_writer.py`**
   ...
   def write_excel(block_data: dict[str, int], output_path: str) -> str:
       """
       Write block count data to Excel file.

       Args:
           block_data: Dictionary of block names to counts
           output_path: Directory or file path for output

       Returns:
           Full path to created Excel file
       """
       logger.info(f"Writing Excel file with {len(block_data)} blocks")

       if not block_data:
           logger.warning("No block data to write")
           raise ValueError("No blocks to export")

       try:
           # Create DataFrame
           df = pd.DataFrame([
               {EXCEL_COLUMN_BLOCK_NAME: name, EXCEL_COLUMN_COUNT: count}
               for name, count in block_data.items()
           ])

           # Sort by count descending
           df = df.sort_values(by=EXCEL_COLUMN_COUNT, ascending=False)
           logger.debug(f"Sorted {len(df)} rows by count")

           # Generate output filename
           timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
           output_file = Path(output_path) / f'blocks_{timestamp}.xlsx'

           # Write Excel file
           logger.debug(f"Writing to: {output_file}")
           with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
               df.to_excel(writer, sheet_name=EXCEL_WORKSHEET_NAME, index=False)

               # Get worksheet and apply auto-filter
...
- [ ] Log messages use appropriate levels (DEBUG/INFO/WARNING/ERROR)
...
3. **Prepare test DWG files** - Have 2-3 real DWG/DXF files for manual testing
4. **Choose development OS** - Windows recommended (primary target platform)

### During Development

1. **Start with Phase 2 (Core Logic)** - Can test independently without GUI
2. **Use test-driven development** - Write tests before/during core implementation
3. **Keep it minimal** - Resist feature creep (no settings, no advanced filtering)
4. **Test with real files** - Don't rely solely on generated test fixtures

### After Implementation

1. **Manual UAT** - Test with 5-10 real DWG files
2. **Performance test** - Verify with files containing 1,000+ blocks
3. **Document limitations** - DWG format support, nested blocks, etc.
4. **Consider packaging** - PyInstaller for easy distribution
