...
Orchestrate sub-agents to sequentially generate and implement specifications in a single flow. Each unit in the document is fully completed (spec generated by one agent, and then implemented by a second agent) before moving to the next unit, allowing later units to learn from actual implementation results.
